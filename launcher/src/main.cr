...
# User-specific config stays in ~/.config/Blue-Environment
...
def die(m)   : NoReturn; err(m); exit(1); end
...
def download_file(url : String, dest : String) : Bool
    ...
    if r.status.success?
        File.write(dest, r.body); ok "Saved"; true
    else
        err "HTTP #{r.status_code}: #{url}"; false
    end
rescue ex
    err "Download failed: #{ex.message}"; false
end
...
def remote_version : String?
    http_get("#{GITHUB_RAW}/launcher/version")&.strip
end
...
def cmd_update
    ...
    # Check permissions
    unless File.writable?(BLUE_SHARE)
        warn "#{BLUE_SHARE} is not writable. Try: sudo blue update"
        exit 1
    end

    FileUtils.mkdir_p(BLUE_LIBS)
    FileUtils.mkdir_p(BLUE_APPS)
    FileUtils.mkdir_p(BLUE_WALLS)
...
def cmd_install(name : String)
    ...
    unless File.writable?(BLUE_APPS)
    ...
    result = Process.run("tar", args: ["-xzf", tmp, "-C", dest],
    ...
    if result.success?
        ...
        Dir.glob(File.join(dest, "**", "*")) do |f|
            ...
            File.chmod(f, 0o755) if File.extname(f).empty? || File.extname(f) == ".sh"
        end
...
def cmd_remove(name : String)
    dest = File.join(BLUE_APPS, name)
    die "'#{name}' is not installed (#{dest} not found)" unless Dir.exists?(dest)
...
def cmd_wallpaper(action : String, arg : String?)
    ...
    when "list"
        ...
        if Dir.exists?(BLUE_WALLS)
            ...
            warn "#{BLUE_WALLS} does not exist"
        end
    when "set"
        ...
        # Write to Blue Environment config
...
def cmd_compositor(action : String)
    case action
    when "start"
        die "Compositor not found at #{COMPOSITOR}" unless File.exists?(COMPOSITOR)
    ...
    when "restart"
        ...
        Process.exec(COMPOSITOR, [] of String)
    when "status"
        result = Process.run("pgrep", args: ["-f", "blue-compositor"], output: Process::Redirect::Pipe)
        if result.exit_code == 0
            ok "Compositor is running"
        else
            warn "Compositor is not running"
        end
    else
...
def print_help
    ...
    puts "    #{cyan("blue start --bigpicture")}          Big Picture (TV) mode"
...
when "install"
    n = args.shift?; n ? cmd_install(n) : (err "Usage: blue install <name>"; exit 1)
when "remove"
    n = args.shift?; n ? cmd_remove(n) : (err "Usage: blue remove <name>"; exit 1)
when "search"
    q = args.shift?; q ? cmd_search(q) : (err "Usage: blue search <query>"; exit 1)
