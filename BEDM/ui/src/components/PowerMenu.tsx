  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 bedm-btn-ghost rounded-xl"
        >
          <X size={16} />
        </button>
        ...
        {confirming ? (
          ...
          <div className="text-center">
            <div
              ...
              style={{
                background: `radial-gradient(circle, ${selected!.glow} 0%, transparent 70%)`,
                border: `2px solid ${selected!.color}30`,
              }}
            >
              <selected!.icon
                size={36}
                style={{ color: selected!.color, filter: `drop-shadow(0 0 8px ${selected!.color})` }}
              />
            </div>
            <div
              className="text-6xl font-light tabular-nums mb-2"
              style={{ fontFamily: '"Oxanium", monospace', color: selected!.color }}
            >
              {countdown}
            </div>
            ...
            <button
              ...
              className="bedm-btn-ghost px-8 py-3 rounded-xl text-sm font-medium"
            >
              Cancel
            </button>
          </div>
          ...
          <>
            <h2
              ...
              style={{ fontFamily: '"Oxanium", monospace', fontSize: '1.25rem', fontWeight: 400 }}
            >
              Power Options
            </h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              {ACTIONS.map(({ action, label, icon: Icon, color, glow, description }) => (
                <button
                  key={action}
                  ...
                  onMouseEnter={e => {
                    (e.currentTarget as HTMLButtonElement).style.background = `${glow.replace('0.3', '0.08')}`;
                    (e.currentTarget as HTMLButtonElement).style.borderColor = `${color}40`;
                    (e.currentTarget as HTMLButtonElement).style.boxShadow = `0 0 24px ${glow}`;
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLButtonElement).style.background = 'rgba(8,20,45,0.6)';
                ...
                >
                  <Icon
                    ...
                    className={`group-hover:!text-[${color}] group-hover:scale-110 transition-all`}
                  />
                  <div>
